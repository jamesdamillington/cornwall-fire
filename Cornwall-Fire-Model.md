...
King’s College London students in the Dept. of Geography. This is a
[Quarto](https://quarto.org/) document that combines text with code -
here we use [the R coding language](https://www.r-project.org/),
primarily with [the *terra*
package](https://rspatial.org/terra/pkg/1-introduction.html) for working
with raster data (see a brief tutorial to Raster GIS operations in R
with *terra* [here](https://www.ecologi.st/spatial-r/terra.html)).
...
(shaded blue) in the *Summary* tab. Several of these calculations are
...
First we read and display the UK CEH Land Cover Map for our study
region. We will use this to determine the location of our fuel types
...
This looks like it has worked, but the map is not very informative, so
let’s change colours and add a proper legend:
...
This looks better. The next line of code will create an interactive map
in R-Studio to show the location of the study area in Cornwall, but not
in the non-interactive version of the document.
...
Now we create raster maps for the curing and biomass parameters,
applying the estimates from the field to each of our land cover types.
First, the curing input (from % dead vegetation) for Grassland and
Heathland.
...
![](Cornwall-Fire-Model_files/figure-commonmark/unnamed-chunk-7-1.png)

Next biomass for Deciduous forest , Conifer forest, Grassland and
Heathland.
...
across the study region we don’t need to make raster maps but can just
...
Later we can vary the values above to examine how different scenarios of
weather influence fire danger and rate of spread of fire.

### Calculating Fire Danger

Now we can calculate Fire Danger using the McArthur grassland fire
danger model for Grassland and Heathland fuel types using their
respective curing and biomass parameter values, and the forest fire
danger model for the Deciduous and Conifer types (based solely on
climate inputs).
...
![](Cornwall-Fire-Model_files/figure-commonmark/unnamed-chunk-11-1.png)

So here’s our first Fire Danger map! Note the scale on the right - the
McArthur model is unitless but can range from 0 (no fire danger) into
the 100s (high danger). Check you can see how the different fuel types
have different danger levels. What land cover (fuel type) are are the
light coloured areas?

### Calculating Rate of Spread

We can also calculate predicted Rate of Spread (RoS) once a fire is
alight. For forest fuel types this is dependent on a combination of fire
danger and biomass, whereas for fuel types based on the grass model
(Grassland, Heathland, Arable) RoS is related only to fire danger.
...
![](Cornwall-Fire-Model_files/figure-commonmark/unnamed-chunk-12-1.png)

### Scenarios

We could now use this model to explore different scenarios, primarily of
different climatic conditions but also for different possible land
use/cover futures.
...
These values should imply a higher fire danger than our original model
application (check you understand why from the values of the variables).
...
And finally apply the model to our map of fuel types to estimate fire
danger and compare to the original Fire Danger map.
...
![](Cornwall-Fire-Model_files/figure-commonmark/unnamed-chunk-15-1.png)

Again, note the colour scale on the right. Where have the light coloured
areas in the original map (on the left) gone?

We can also compare summaries of the values in each of our Fire Danger
maps:
...
![](Cornwall-Fire-Model_files/figure-commonmark/unnamed-chunk-17-1.png)

As above, check you understand how the colour scale as affected the
original map. Comparing summaries of the values in the raster maps might
help.
...
Try downloading this document and opening with R-Studio to work with the
code interactively. Try changing climate variable values to see the
effect on Fire Danger and Rate of Spread across the study area. An
interactive version of this (online) will hopefully be ready soon.
